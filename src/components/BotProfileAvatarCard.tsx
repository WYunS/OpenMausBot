...
const CROP_LABEL_KEY = {
  mascot: "botSettings.avatar.crop.mascot",
  circle: "botSettings.avatar.crop.circle",
  rounded: "botSettings.avatar.crop.rounded",
  square: "botSettings.avatar.crop.square",
  ...
  return (
    <div className="overflow-hidden rounded-xl border border-hairline/40 bg-card">
      <div className="flex items-center justify-between border-b border-hairline/40 px-3 py-2.5">
        <span className="rounded-lg bg-control px-3 py-1.5 text-[14px] font-medium text-ink">{t("botSettings.avatar.title")}</span>
        <button
      ...
      <div className="p-3">
        ...
        <div className="mt-2 flex gap-2">
          ...
          {bot.avatarUrl && (
            <button
              ...
              onClick={removeImage}
              disabled={uploading || generating}
              aria-label={t("botSettings.avatar.removeAria")}
              title={t("botSettings.avatar.remove")}
        ...
        <div className="mt-1.5 text-[11.5px] text-ink-secondary">{t("botSettings.avatar.fileHint")}</div>

        <div className="mb-2 mt-4 text-[12px] font-medium uppercase tracking-[0.08em] text-ink-secondary">
          {t("botSettings.avatar.shape")}
        </div>
        ...
        {crop === "mascot" && (
          <>
            <div className="mb-2 mt-4 text-[12px] font-medium uppercase tracking-[0.08em] text-ink-secondary">
              {t("botSettings.avatar.expression")}
            </div>
        ...
        <div className="mt-5 border-t border-hairline/40 pt-4">
          <div className="flex items-center gap-2 text-[13px] font-medium text-ink">
            <Sparkles size={14} className="text-accent" /> {t("botSettings.avatar.generateTitle")}
          </div>
          <div className="mt-1 text-[11.5px] leading-relaxed text-ink-secondary">
            {t("botSettings.avatar.generateHelp")}
          </div>
            ...
            <div className="mt-3">
              <div className="flex gap-2">
                <input
                  ...
                  placeholder={t("botSettings.avatar.apiKeyPlaceholder")}
                ...
                >
                  {savingKey ? <Loader2 size={13} className="animate-spin" /> : <><Check size={13} /> {t("botSettings.avatar.save")}</>}
                </button>
              </div>
              <div className="mt-1.5 text-[11px] text-ink-secondary">{t("botSettings.avatar.keyStorageHelp")}</div>
            </div>
          ) : (
            <div className="mt-3">
              <textarea
                ...
                maxLength={400}
                placeholder={t("botSettings.avatar.directionPlaceholder", { name: bot.title || bot.name })}
                aria-label={t("botSettings.avatar.directionAria")}
              ...
              <div className="mt-2 flex items-center justify-between gap-3">
                ...
                >
                  ...
                  {generating ? t("botSettings.avatar.generating") : t("botSettings.avatar.generate")}
                </button>
              </div>
              <details className="mt-3 rounded-lg border border-hairline/40 bg-inset px-3 py-2">
                <summary className="cursor-pointer text-[11.5px] text-ink-secondary">{t("botSettings.avatar.replaceKey")}</summary>
                <div className="mt-2 flex gap-2">
                  <input
                    type="password"
                    value={imageKey}
                    onChange={(event) => setImageKey(event.target.value)}
                    onKeyDown={(event) => event.key === "Enter" && void saveImageKey()}
                    placeholder={t("botSettings.avatar.replacementKeyPlaceholder")}
                    aria-label={t("botSettings.avatar.replacementKeyAria")}
                    autoComplete="off"
