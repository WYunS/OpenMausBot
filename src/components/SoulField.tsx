    ...
    <div className="block">
      <div className="mb-1.5 flex items-center justify-between gap-3">
        ...
        {canMigrate && (
          ...
          >
            {t("botSettings.soul.move")}
          </button>
        )}
      </div>
      {info?.drift && (
        <div className="mb-2 rounded-lg border border-amber-500/40 bg-amber-500/10 p-3 text-[12px] text-ink">
          <div className="font-medium">{t("botSettings.soul.driftTitle")}</div>
          <div className="mt-1 text-ink-secondary">
            {t("botSettings.soul.driftBefore")} <span className="break-all">{info.file}</span>
          </div>
      ...
      <textarea
        ...
        placeholder={t("botSettings.soul.placeholder")}
      ...
      <div className="mt-1.5 flex items-start justify-between gap-3 text-[11px] text-ink-secondary">
        <span>
          {t("botSettings.soul.context")}{info ? <> {t("botSettings.soul.mirroredTo")} <span className="break-all">{info.file}</span>。</> : null}
        </span>
        <span className={cn("shrink-0 tabular-nums", over && "font-medium text-red-500")}>
          {t("botSettings.soul.bytes", { bytes: bytes.toLocaleString(), limit: limit.toLocaleString() })}{over ? t("botSettings.soul.notSaved") : ""}
        </span>
      </div>
    </div>
  );
