    ...
    <div className="rounded-xl bg-card p-4">
      ...
      >
        <span className="text-[15px] font-medium text-ink">
          {data ? t("botSettings.prompt.summary", {
            bytes: data.totalBytes.toLocaleString(),
            tokens: data.approxTokens.toLocaleString(),
          }) : t("botSettings.prompt.title")}
        </span>
        <ChevronDown size={16} className={cn("shrink-0 text-ink-secondary transition-transform", open && "rotate-180")} />
      </button>

      {open && !data && error && (
        <div className="mt-3 text-[13px] text-ink-secondary">{t("botSettings.prompt.error")}</div>
      )}

      {open && !data && !error && <div className="mt-3 text-[13px] text-ink-secondary">{t("botSettings.overview.loading")}</div>}
      ...
      {open && data && (
        <div className="mt-3 flex flex-col gap-2">
          {data.sections.map((section) => (
            <details key={section.id} className="rounded-lg border border-hairline/40 bg-inset px-3 py-2">
              <summary className="flex cursor-pointer items-center justify-between gap-3 text-[13px] text-ink">
                <span>{section.id === "persona" ? t("botSettings.prompt.identity") : section.id === "soul" ? t("botSettings.prompt.soul") : section.label}</span>
                <span className="shrink-0 tabular-nums text-ink-secondary">{t("botSettings.prompt.bytes", { bytes: section.bytes.toLocaleString() })}</span>
              </summary>
          ...
          {data.note && <div className="text-[11px] text-ink-secondary">{t("botSettings.prompt.note")}</div>}
        </div>
      )}
    </div>
  );
