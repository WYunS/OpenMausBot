...
import { localizeOverviewLine } from "@/lib/bot-overview-localization";
    ...
    <div className="flex flex-col gap-4">
      {refreshError && (
        <div className="rounded-lg bg-inset px-3 py-2 text-[12.5px] text-ink-secondary">
          {t("botSettings.overview.refreshError")}
        </div>
      )}
      ...
      <div className="rounded-xl bg-card p-4">
        ...
        {overview.who.soulLead && (
          <div className="mt-3 rounded-lg bg-inset px-3 py-2.5">
            ...
            >
              {t("botSettings.overview.readAll")}
            </button>
          </div>
        )}
      </div>
      ...
      <div className="rounded-xl bg-card p-4">
        <div className="text-[15px] font-medium text-ink">{t("botSettings.overview.does")}</div>
        {overview.does.length === 0 ? (
          <p className="mt-2 text-[13px] text-ink-secondary">{t("botSettings.overview.doesEmpty")}</p>
        ) : (
          <ul className="mt-2 flex flex-col gap-1.5 text-[13px] leading-relaxed text-ink">
            {overview.does.map((line, i) => (
              <li key={i}>{localizeOverviewLine(line)}</li>
            ))}
          </ul>
        )}
      </div>
      ...
      <div className="rounded-xl bg-card p-4">
        <div className="text-[15px] font-medium text-ink">{t("botSettings.overview.reaches")}</div>
        {overview.reaches.length === 0 ? (
          <p className="mt-2 text-[13px] text-ink-secondary">{t("botSettings.overview.reachesEmpty")}</p>
        ) : (
          <ul className="mt-2 flex flex-col gap-1.5 text-[13px] leading-relaxed text-ink">
      ...
      <div className="rounded-xl bg-card p-4">
        <div className="text-[15px] font-medium text-ink">{t("botSettings.overview.wont")}</div>
        <ul className="mt-2 flex flex-col gap-1.5 text-[13px] leading-relaxed text-ink">
      ...
      <div className="rounded-xl bg-card p-4">
        <div className="flex items-baseline justify-between gap-3">
          <div className="text-[15px] font-medium text-ink">{t("botSettings.overview.recent")}</div>
          <button
            type="button"
            onClick={() => onOpen("history")}
            className="shrink-0 text-[12px] text-ink-secondary hover:text-ink"
          >
            {t("botSettings.overview.viewAll")}
          </button>
        </div>
        {overview.recent.length === 0 ? (
          <p className="mt-2 text-[13px] text-ink-secondary">{t("botSettings.overview.recentEmpty")}</p>
        ) : (
