...
function WorkingFolder({ bot }: { bot: Bot }) {
  ...
  return (
    <div className="rounded-xl bg-card p-4">
      ...
      <div className="mt-0.5 text-[13px] text-ink-secondary">{t("botSettings.access.folderHelp")}</div>
      {canPick ? (
      ...
      {pinnedElsewhere && (
        <div className="mt-2 text-[12px] text-ink-secondary">
          {t("botSettings.access.pinnedBefore")} {pinned ? <span className="font-mono">{shortPath(pinned, home)}</span> : t("botSettings.access.homeFolder")} {t("botSettings.access.pinnedAfter")}
        </div>
      )}
    </div>
  );
    ...
    <div className="flex flex-col gap-4">
      <div className="rounded-xl bg-card p-4">
        ...
        <div className="mt-0.5 text-[13px] text-ink-secondary">
          {t(bot.computer ? "botSettings.access.worksOnHelp" : "botSettings.access.worksOnHelpAuto")}
        </div>
        <div className="mt-3 flex overflow-hidden rounded-lg border border-hairline/40">
          {([
            [null, t("botSettings.access.mode.auto")],
            ["cloud", t("botSettings.access.mode.cloud")],
            ["vm", t("botSettings.access.mode.vm")],
            ["local", t("botSettings.access.mode.local")],
            ["browser", t("botSettings.access.mode.browser")],
            ["off", t("botSettings.access.mode.off")],
            ...
            <button
              ...
              title={
                mode === "local" && !localSelectable
                  ...
                  : mode === "browser"
                    ? browserSelectable ? t("botSettings.access.browserOnly") : browserDisabledReason
                    : undefined
              }
              onClick={() => {
          ...
          <>
            {!bot.computer && (
              <div className="mt-3 rounded-lg bg-inset px-3 py-2.5 text-[11.5px] leading-relaxed text-ink-secondary">
                <span className="font-medium text-ink">{t("botSettings.access.autoCloud")}</span>{" "}
                {t("botSettings.access.autoCloudHelp")}
              </div>
            )}
            <CloudBackendPicker
            ...
            {!bot.computer && bot.cloudBackend === "vps" && (
              <div className="mt-3 flex items-center justify-between gap-4 rounded-lg bg-inset px-3 py-2.5">
                <div className="min-w-0">
                  <div className="text-[13px] text-ink">{t("botSettings.access.autoVps")}</div>
                  <div className="mt-0.5 text-[11.5px] text-ink-secondary">
                    {t("botSettings.access.autoVpsHelp")}
                  </div>
                </div>
                <Switch
                  checked={Boolean(bot.autoStartVps)}
                  aria-label={t("botSettings.access.autoVps")}
      ...
      <div className="rounded-xl bg-card p-4">
        <div className="flex items-center justify-between gap-4">
          <div>
            <div className="text-[15px] font-medium text-ink">{t("botSettings.access.apps")}</div>
            <div className="mt-0.5 text-[13px] text-ink-secondary">
              {!connectedAppsConfigured
                ? t("botSettings.access.appsConfigure")
                : !canUseConnectedApps
                  ? t("botSettings.access.appsUnsupported")
                  : connectedAppsEnabled
                    ? t("botSettings.access.appsEnabled")
                    : t("botSettings.access.appsDisabled")}
            </div>
          </div>
          <Switch
            ...
            title={
              !connectedAppsEnabled && !connectedAppsConfigured
                ? t("botSettings.access.appsConfigureFirst")
                : !connectedAppsEnabled && !canUseConnectedApps
                  ? t("botSettings.access.appsEngineUnsupported")
                  : undefined
            }
        ...
        {connectedAppsEnabled && inventory?.authoritative && (
          <div className="mt-3 flex flex-wrap gap-1.5">
            {connectedSlugs.length === 0 ? (
              <span className="text-[11.5px] text-ink-secondary">{t("botSettings.access.appsEmpty")}</span>
            ) : (
          ...
          <div className="mt-0.5 text-[13px] text-ink-secondary">
            {!desktopBrowser
              ? browserBlockedOnWindows && !browserInstallable
                ? t("botSettings.access.browserInstall")
                : browserUnavailableReason(state.config)
              : !browserFeature
                ? t("botSettings.access.browserFeatureOff")
                : !canUseBrowser
                  ? t("botSettings.access.browserUnsupported")
                  : browserEnabled
                    ? t("botSettings.access.browserEnabled")
                    : t("botSettings.access.browserDisabled")}
          </div>
        </div>
        <Switch
      ...
      <div className="rounded-xl bg-card p-4">
        ...
        <div className="mt-0.5 text-[13px] text-ink-secondary">{t("botSettings.access.webhooksHelp")}</div>
        {webhooks.length === 0 ? (
          ...
          <div className="mt-3 divide-y divide-hairline/40 overflow-hidden rounded-lg border border-hairline/40">
            {webhooks.map((webhook) => (
              <div key={webhook.id} className="flex items-center justify-between gap-3 px-3 py-2">
                ...
                >
                  {webhook.enabled ? t("common.active") : t("common.paused")}
                </span>
      ...
      <div className="rounded-xl bg-card p-4">
        ...
        <div className="mt-0.5 text-[13px] text-ink-secondary">{t("botSettings.access.alwaysAllowedHelp")}</div>
        {alwaysAllow.length === 0 ? (
          <div className="mt-3 rounded-lg bg-inset px-3 py-2 text-[12px] text-ink-secondary">{t("botSettings.access.alwaysAllowedEmpty")}</div>
        ) : (
          <div className="mt-3 divide-y divide-hairline/40 overflow-hidden rounded-lg border border-hairline/40">
            {alwaysAllow.map((entry) => (
              <div key={entry} className="flex items-center justify-between gap-3 px-3 py-2">
                ...
                <button
                  type="button"
                  aria-label={t("botSettings.access.removeAllowedAria", { name: entry })}
