...
export function SkillsSection({ bot }: { bot: Bot }) {
  ...
  const refresh = async (cancelled?: () => boolean) => {
    try {
      ...
      if (!cancelled?.()) setError(cause instanceof Error ? cause.message : t("botSettings.skills.loadError"));
    } finally {
  ...
  const toggle = async (skill: ManagedSkill) => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : t("botSettings.skills.updateError"));
    } finally {
      setWorking("");
    }
  };
  ...
  const enableReviewed = async () => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : t("botSettings.skills.enableError"));
    } finally {
      setWorking("");
    }
  };
  ...
  const remove = async (skill: ManagedSkill) => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : t("botSettings.skills.removeError"));
    } finally {
      setWorking("");
    }
  };
  ...
  const view = async (skill: ManagedSkill) => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : t("botSettings.skills.loadOneError"));
    }
  };
  ...
  const importSkill = async () => {
    ...
    try {
      ...
      setError(cause instanceof Error ? cause.message : t("botSettings.skills.importError"));
    } finally {
      setImporting(false);
    }
  };
    ...
    <div className="flex flex-col gap-4">
      <div className="rounded-xl bg-card p-4">
        ...
        <div className="mt-1 text-[12px] leading-relaxed text-ink-secondary">
          {featureEnabled
            ? t("botSettings.skills.helpOn")
            : t("botSettings.skills.helpOff")}
        </div>
        ...
        {loading ? (
          <div className="mt-3 text-[12px] text-ink-secondary">{t("botSettings.overview.loading")}</div>
        ) : skills.length === 0 ? (
              ...
              <div key={skill.name} className="px-3 py-2.5">
                <div className="flex items-center gap-2">
                  ...
                  >
                    ...
                    <div className="mt-0.5 text-[10.5px] text-ink-secondary">{t("botSettings.skills.usedWhenRelevant")}</div>
                  </button>
                  <Switch
                  ...
                  <button
                    ...
                    title={t("botSettings.skills.remove")}
                    disabled={working === skill.name}
        ...
        {staged.length > 0 && (
          <div className="mt-2 text-[11.5px] text-warning">
            {t("botSettings.skills.proposals", { count: staged.length })}
          </div>
        )}
      ...
      {reviewing && (
        ...
        >
          <div className="flex max-h-[min(760px,90vh)] w-full max-w-2xl flex-col rounded-2xl bg-card p-5 shadow-2xl">
            ...
            <pre
              tabIndex={0}
              aria-label={t("botSettings.skills.fullAria", { name: reviewing.skill.name })}
      ...
      {viewing && (
        ...
        >
          <div className="flex max-h-[min(760px,90vh)] w-full max-w-2xl flex-col rounded-2xl bg-card p-5 shadow-2xl">
            ...
            <pre
              tabIndex={0}
              aria-label={t("botSettings.skills.fullAria", { name: viewing.name })}
              className="mt-3 min-h-0 flex-1 overflow-auto whitespace-pre-wrap break-words rounded-xl bg-inset p-3 font-mono text-[12px] leading-relaxed text-ink"
            >
              {viewing.text}
            </pre>
