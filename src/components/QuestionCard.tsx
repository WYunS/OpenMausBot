...
export function QuestionCard({
  ...
  const submit = () => {
    ...
    dispatch({
      ...
      behavior: "answer",
      message: answer,
      answers: questions.every(question => question.id) ? questions.map((question, index) => {
        const draft = drafts[index] ?? EMPTY;
        return { id: question.id!, selected: draft.picked,
          ...(draft.other && draft.custom.trim() ? { custom: draft.custom.trim() } : {}) };
      }) : undefined,
  ...
  return (
    ...
    >
      ...
      {current.detail && <div className="mt-2 max-h-80 overflow-auto whitespace-pre-wrap break-words rounded-lg bg-inset p-3 text-[13px] text-ink-secondary">{current.detail}</div>}
      ...
      {settled ? (
        ...
        <div className="mt-3 flex items-center justify-end gap-3">
          <button type="button" className="mr-auto text-[13px] text-ink-secondary hover:text-ink" onClick={() => {
            if (!card.requestId || settled) return;
            setSent(t("question.cancelled"));
            dispatch({ type: "decideRequest", threadId, requestId: card.requestId, behavior: "answer", cancelQuestion: true, onError: () => setSent(null) });
          }}>{t("question.cancel")}</button>
