...
export function CalendarSidebar({ bots, anchor, onSelectDate }: CalendarSidebarProps) {
  ...
  return (
    <aside
      aria-label={t("automation.sidebarAria")}
    ...
    >
      ...
      <section aria-labelledby="calendar-bots-heading" className="flex min-h-0 flex-1 flex-col px-3 pb-3 pt-3">
        ...
        <label className="relative mb-2 block">
          <span className="sr-only">{t("automation.searchBots")}</span>
          <Search
            size={13}
          ...
          <input
            ...
            type="search"
            placeholder={t("automation.searchBots")}
        ...
        <div role="list" aria-label={t("automation.availableBots")} className="min-h-0 flex-1 space-y-0.5 overflow-y-auto pr-0.5">
          {filteredBots.map((bot) => (
            <div
              ...
              aria-label={t("automation.dragBotAria", { name: bot.name })}
              title={t("automation.dragBotAria", { name: bot.name })}
            >
              <GripVertical
                size={13}
          ...
          {filteredBots.length === 0 && (
            <div className="px-3 py-8 text-center text-[11px] leading-relaxed text-ink-secondary">
              {bots.length === 0 ? t("automation.createBotFirst") : t("automation.noBotsMatch")}
            </div>
          )}
        </div>

        <p className="mt-2 px-2 text-[9.5px] leading-relaxed text-ink-secondary/65">
          {t("automation.dragHelp")}
        </p>
      </section>
    </aside>
  );
