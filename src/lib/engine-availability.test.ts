...
describe("user-controlled engine availability", () => {
  it("defaults every built-in engine on while honoring an explicit disable", () => {
    expect(engineSelectable("ruijieHarness")).toBe(true);
    expect(engineSelectable("codex")).toBe(true);
    expect(engineSelectable("claudeAgent")).toBe(true);
    expect(engineSelectable("antigravityAgent")).toBe(true);
