...
```sh
umask 077
tar czf openmausbot-data.tgz -C "$HOME" .openmausbot
```
...
docker run --rm -v deploy_data:/data -v "$PWD":/b alpine sh -c 'umask 077; tar czf /b/openmausbot-data.tgz -C /data .'
```

Keep backups privately on another machine. Restore with the server stopped,
