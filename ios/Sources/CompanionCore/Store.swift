...
public struct CompanionState: Sendable {
    ...
    public mutating func hydrate(_ fleet: Fleet, waitingThreads: [String: ThreadPage] = [:]) {
        ...
        // settled behind its back, so rows it is still holding may have
        ...
        case let .bot(bot):
            ...
            if let index = bots.firstIndex(where: { $0.id == bot.id }) {
                ...
                if let replacement = bot.messages {
                    ...
                    // retires a row. Without this the held message stays above
                    // the chat bar for ever, long after its line has landed.
                ...
                // `currentTaskBusy`, not `busy`: with independent bot threads a
                // bot working on another thread must not keep this one's
                // half-written answer streaming.
                if bot.currentTaskBusy != true { clearStream(bot.threadId) }
            } else {
                bots.append(bot)
