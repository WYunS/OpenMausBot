...
import { signAdHoc, verifyAdHocUniversal, run } from "./enterprise-macos-sign.mjs";
...
writeFileSync(join(outputApp, "Contents/Resources/app-update.yml"), stringify({
  ...enterpriseConfig.publish[0],
  updaterCacheDirName: `${packageName}-updater`,
}));
execFileSync(process.execPath, ["scripts/check-enterprise-package.mjs", join(outputApp, "Contents/Resources")], { stdio: "inherit" });
...
console.log("Universal DMG complete: ad-hoc signed, not notarized; internal testing only.");
