...
export function desktopRuntimeLayout({ packaged, preview, appRoot, resourcesPath, platform, arch, resolveResourceRoot = realpathSync }) {
  ...
  const harnessRoot = packaged ? path.join(resourcesPath, 'ruijie-harness') : undefined;
  const universalHarness = packaged && platform === 'darwin' && existsSync(path.join(harnessRoot, 'universal.json'));
  return {
    built,
    ...
    harness: packaged ? universalHarness ? path.join(harnessRoot, `${platform}-${arch}`) : harnessRoot
