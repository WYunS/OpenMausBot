...
本版本不依赖锐捷沙箱。首次启动及以后每次新建 Bot，`computer` 都必须默认为 `local`，
直接使用安装包内置 CUA 控制当前 Windows 电脑；旧 Bot 的既有电脑选择不迁移、不覆盖。

- `pnpm package:win` 不读取、生成或打包 `release-inputs/ruijie-sandbox.json`，也不要求 manager URL、
  request JSON、client_id、vnc_key 或 minio_url。沙箱仍可由用户以后主动配置，但不是本版交付前提。
- 公开源码和安装包不得包含账号、Token、Cookie、飞书凭据、模型密钥、VPN 配置或用户数据。
- 全新隔离用户验收：连续新建两个 Bot，均显示“这台电脑”；退出重开后选择不回退；
  截图、点击、键盘输入、仅真实用户输入触发人工接管、交还后继续执行均须通过。
- CUA 必须来自候选安装包的 `resources`，不得借开发机 PATH 或另一份已安装驱动通过。
...
2026-09-14 本地回退修复：编译预览也必须导入同一份预置，不能仅用 `app.isPackaged` 判断。
同一预置地址已经存在但凭据缺失时补齐；其他地址、已保存凭据和显式清除标记仍保留。
预览的凭据保存与安装版共用加密存储、私有请求授权和落盘去明文路径。验收必须覆盖实际客户端连续重开，
不能只看工具进程中的配置文件或把命令行网络探测当作客户端自身的连通性。
