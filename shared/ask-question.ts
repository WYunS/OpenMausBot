...
export interface AskQuestion {
  /** Native Harness correlation. Never derive it from the question text. */
  id?: string;
  detail?: string;
  intent?: { kind: string };
  question: string;
...
/** Native protocol: preserve every ID, label and position. Oversized or
 * malformed batches fail explicitly, never silently truncate into another ask. */
export function parseHarnessQuestions(input: unknown): AskQuestion[] | null {
  ...
  for (const raw of input) {
    if (!isRecord(raw) || !validText(raw.id, 512) || seen.has(raw.id) || !validText(raw.question, 16_384)) return null;
    if (raw.detail !== undefined && (typeof raw.detail !== "string" || raw.detail.length > 262_144)) return null;
    if (raw.header !== undefined && !validText(raw.header, 1024)) return null;
    if (raw.multiSelect !== undefined && typeof raw.multiSelect !== "boolean") return null;
    if (raw.options !== undefined && (!Array.isArray(raw.options) || raw.options.length > 64)) return null;
    ...
    for (const option of raw.options ?? []) {
      ...
      if (!validText(label, 16_384) || options.some(item => item.label === label)) return null;
      if (description !== undefined && (typeof description !== "string" || description.length > 16_384)) return null;
      options.push({ label, ...(description === undefined ? {} : { description }) });
    }
    seen.add(raw.id);
    result.push({ id: raw.id, question: raw.question, options,
      ...(typeof raw.header === "string" ? { header: raw.header } : {}),
      ...(typeof raw.detail === "string" ? { detail: raw.detail } : {}),
      ...(raw.multiSelect === true ? { multiSelect: true } : {}),
      ...(isRecord(raw.intent) && typeof raw.intent.kind === "string" ? { intent: { kind: raw.intent.kind } } : {}),
    });
  }
  return result;
}

export function validateQuestionAnswers(questions: readonly AskQuestion[], input: unknown): QuestionAnswer[] | null {
  if (!Array.isArray(input) || input.length !== questions.length) return null;
  const answers: QuestionAnswer[] = [];
  for (const [index, question] of questions.entries()) {
    const raw = input[index];
    if (!isRecord(raw) || !question.id || raw.id !== question.id || !Array.isArray(raw.selected)) return null;
    if (raw.selected.some(label => typeof label !== "string" || !question.options.some(option => option.label === label))
      || new Set(raw.selected).size !== raw.selected.length) return null;
    if (raw.custom !== undefined && (typeof raw.custom !== "string" || !raw.custom.trim() || raw.custom.length > MAX_CUSTOM_ANSWER)) return null;
    if (!question.multiSelect && (raw.selected.length > 1 || (raw.selected.length > 0 && raw.custom !== undefined))) return null;
    answers.push({ id: question.id, selected: [...raw.selected], ...(typeof raw.custom === "string" ? { custom: raw.custom.trim() } : {}) });
  }
  return answers;
}

/** Compatibility for old clients: only a single question has an unambiguous flat answer. */
export function legacyQuestionAnswer(questions: readonly AskQuestion[], message: unknown): QuestionAnswer[] | null {
  const question = questions.length === 1 ? questions[0] : undefined;
  if (!question?.id || typeof message !== "string" || !message.trim()) return null;
  return validateQuestionAnswers(questions, [{ id: question.id,
    selected: question.options.some(option => option.label === message) ? [message] : [],
    ...(question.options.some(option => option.label === message) ? {} : { custom: message }),
  }]);
}
