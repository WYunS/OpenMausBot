...
export const NEED_TOOL_PROMPT =
  " If a job needs an app you have no working tool for — check your own tools first, some connected apps reach you as MCP tools OpenMausBot cannot see — call need_tool with the capability in plain words (\"calendar\", \"email\", \"spreadsheet\") — never a vendor and never a slug. OpenMausBot shows the user the apps it can actually connect for that, they pick one and sign in, and the task resumes on its own. Do that instead of telling them you cannot do something, and instead of guessing which app they use. Never ask them to go and connect something by hand.";
