...
describe("Ruijie authenticated CUA computer proxy", () => {
  let bridge: Server;
  let proxy: ChildProcess;
  const requests: any[] = [];
  const results = new Map<number, any>();
  ...
  beforeAll(async () => {
    bridge = createServer((req, res) => {
      ...
      req.on("end", () => {
        ...
        const result = message.method === "tools/list"
          ? { tools: [{ name: "get_desktop_state", description: "official CUA", inputSchema: { type: "object" } }] }
          : message.method === "tools/call"
            ? { content: [{ type: "text", text: "official click" }] }
            : { protocolVersion: "2024-11-05", capabilities: { tools: {} }, serverInfo: { name: "cua-driver" } };
        res.writeHead(200, { "content-type": "application/json" });
    ...
    proxy = spawn(process.execPath, ["--experimental-strip-types", PROXY], {
      env: {
        ...process.env,
        OMB_RUIJIE_CUA_URL: `http://127.0.0.1:${port}`,
        OMB_RUIJIE_CUA_TOKEN: TOKEN,
      },
      stdio: ["pipe", "pipe", "pipe"],
    });
    let pending = "";
  ...
  it("forwards official MCP calls with bearer auth and keeps human takeover", async () => {
    rpc({ jsonrpc: "2.0", id: 10, method: "initialize", params: {} });
    expect((await waitFor(10)).result.serverInfo.name).toBe("cua-driver");

    rpc({ jsonrpc: "2.0", id: 11, method: "tools/list" });
    expect((await waitFor(11)).result.tools.map((tool: any) => tool.name)).toEqual([
      "get_desktop_state", "computer_request_help",
    ]);

    rpc({ jsonrpc: "2.0", id: 12, method: "tools/call", params: { name: "click", arguments: { x: 1, y: 2 } } });
    expect((await waitFor(12)).result.content[0].text).toBe("official click");
