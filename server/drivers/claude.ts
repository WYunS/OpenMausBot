          ...
          broker = await createPermissionBroker({
            ...
            onAsk: (ask) => {
              ...
              emit({
                ...
                // Claude can retain its own suggested rules for the session;
                // host control deliberately remains one action at a time.
                allowSession:
                  ask.kind === "permission" &&
                  !(controlsHost && typeof ask.tool === "string" && ask.tool.startsWith("mcp__computer"))
                    ? true
                    : undefined,
                approvalScope:
